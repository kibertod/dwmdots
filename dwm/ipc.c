 ...
 */
static int ipc_recv_message(
    int fd, uint8_t* msg_type, uint32_t* reply_size, uint8_t** reply) {
    ...
    while (read_bytes < to_read) {
        ...
        if (n == 0) {
            if (read_bytes == 0) {
                fprintf(
                    stderr, "Unexpectedly reached EOF while reading header.");
                fprintf(stderr,
 ...
 * handle.
 */
static void ipc_reply_prepare_send_message(
    yajl_gen gen, IPCClient* c, IPCMessageType msg_type) {
    const unsigned char* buffer;
    size_t len = 0;
 ...
 */
static int ipc_validate_run_command(
    IPCParsedCommand* parsed, const IPCCommand actual) {
 ...
 * Returns -1 otherwise
 */
static int ipc_parse_subscribe(
    const char* msg, IPCSubscriptionAction* subscribe, IPCEvent* event) {
...
static int ipc_run_command(IPCClient* ipc_client, char* msg) {
    ...
    if (ipc_parse_run_command(msg, &parsed_command) < 0) {
        ipc_prepare_reply_failure(
            ipc_client, IPC_TYPE_RUN_COMMAND, "Failed to parse run command");
        return -1;
    }
    ...
    if (res < 0) {
        if (res == -1)
            ...
            ipc_prepare_reply_failure(
                ipc_client, IPC_TYPE_RUN_COMMAND, "Type mismatch");
        ipc_free_parsed_command_members(&parsed_command);
        return -1;
    }
 ...
 */
static void ipc_get_layouts(
    IPCClient* c, const Layout layouts[], const int layouts_len) {
    yajl_gen gen;
    ipc_reply_init_message(&gen);
 ...
 */
static int ipc_get_dwm_client(
    IPCClient* ipc_client, const char* msg, const Monitor* mons) {
    Window win;
        ...
        for (Client* c = m->clients; c; c = c->next)
            if (c->win == win) {
                ...
                ipc_reply_prepare_send_message(
                    gen, ipc_client, IPC_TYPE_GET_DWM_CLIENT);
...
static int ipc_subscribe(IPCClient* c, const char* msg) {
    ...
    if (ipc_parse_subscribe(msg, &action, &event)) {
        ipc_prepare_reply_failure(
            c, IPC_TYPE_SUBSCRIBE, "Event does not exist");
        return -1;
    }
    ...
    if (action == IPC_ACTION_SUBSCRIBE) {
        ...
        ipc_prepare_reply_failure(
            c, IPC_TYPE_SUBSCRIBE, "Invalid subscription action");
        return -1;
    }
...
int ipc_read_client(
    IPCClient* c, IPCMessageType* msg_type, uint32_t* msg_size, char** msg) {
    int fd = c->fd;
    int ret =
...
void ipc_prepare_reply_failure(
    IPCClient* c, IPCMessageType msg_type, const char* format, ...) {
    yajl_gen gen;
    va_list args;
...
void ipc_client_focus_change_event(
    int mon_num, Client* old_client, Client* new_client) {
    yajl_gen gen;
    ipc_event_init_message(&gen);
...
void ipc_layout_change_event(const int mon_num, const char* old_symbol,
    ...
    dump_layout_change_event(
        gen, mon_num, old_symbol, old_layout, new_symbol, new_layout);
    ipc_event_prepare_send_message(gen, IPC_EVENT_LAYOUT_CHANGE);
}

void ipc_monitor_focus_change_event(
    const int last_mon_num, const int new_mon_num) {
    yajl_gen gen;
    ipc_event_init_message(&gen);
...
void ipc_focused_title_change_event(const int mon_num, const Window client_id,
    ...
    yajl_gen gen;
    ipc_event_init_message(&gen);
    dump_focused_title_change_event(
        gen, mon_num, client_id, old_name, new_name);
...
void ipc_focused_state_change_event(const int mon_num, const Window client_id,
    ...
    yajl_gen gen;
    ipc_event_init_message(&gen);
    dump_focused_state_change_event(
        gen, mon_num, client_id, old_state, new_state);
...
void ipc_send_events(Monitor* mons, Monitor** lastselmon, Monitor* selmon) {
    for (Monitor* m = mons; m; m = m->next) {
        ...
        TagState new_state = {
            .selected = tagset, .occupied = occ, .urgent = urg
        };
...
int ipc_handle_client_epoll_event(struct epoll_event* ev, Monitor* mons,
    ...
    if (ev->events & EPOLLHUP) {
        ...
        if (msg_type == IPC_TYPE_GET_MONITORS)
            ...
            ipc_prepare_reply_failure(
                c, msg_type, "Invalid message type: %d", msg_type);
        }
        free(msg);
    } else {
